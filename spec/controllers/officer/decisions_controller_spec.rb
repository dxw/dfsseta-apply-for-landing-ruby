...
RSpec.describe Officer::DecisionsController do
  ...
  describe "POST to :create" do
    ...
    it "doesn't save the applicatind and renders :new" do
      form = instance_double(AssessmentDecisionForm, "valid?" => true, "application_decision" => "proved")
      allow(AssessmentDecisionForm).to receive(:new).and_return(form)

      landing_application = instance_double(LandingApplication, "valid?" => false, :save => double, "application_decision=" => "proved")
      allow(LandingApplication).to receive(:find).and_return(landing_application)
